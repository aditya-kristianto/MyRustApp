...
use super::repository::Mail::MailHistory;
use tokio_postgres::Client;

pub fn configure_v1(config: &mut ServiceConfig) {
    config
        .service(get_mail_histories);
}

/// Get Mail History
///
/// To get mail history list
#[utoipa::path(
    get,
    context_path = "/v1",
    path = "/mail/histories",
...
#[get("/v1/mail/histories")]
pub async fn get_mail_histories(
    ...
    let mail_histories = match MailHistory::get_mail_histories(data.clone()).await {
        Ok(summary) => summary,
        Err(err) => {
            let resp = Response::new(
                None,
                None,
                Some(format!("Error fetching mail history: {}", err)),
                None,
    ...
    let mail_histories_json: Vec<String> = mail_histories.iter()
        .map(|e| to_string(e).unwrap()) // Convert each Mail History to JSON string
        .collect();

    // Construct response with meta data
    let resp = Response::new(
        Some(HashMap::from([(
            "mail history".to_string(),
            DataValue::StringArray(mail_histories_json),
        )])),
        None,
        None,
        Some(Meta {
