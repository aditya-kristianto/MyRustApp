...
if [ -n "${IS_BUILD_RUST_ALPINE}" ] && [ "${IS_BUILD_RUST_ALPINE}" == "true" ]; then
    ...
    if [ -z "$latest_image" ]; then
        ...
        docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ARCH}/${IMAGE_NAME}:1.0.0-${TARGET}
    else
...
if [ -n "${IS_ECR}" ] && [ "${IS_ECR}" == "true" ]; then
    ...
    if [ -z "$rust_alpine_latest_image" ]; then
        ...
        docker push ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ARCH}/${RUST_ALPINE_IMAGE_NAME}:1.0.0-${TARGET}
