...
pub fn configure() -> impl FnOnce(&mut ServiceConfig) {
    |config: &mut ServiceConfig| {
        config.service(get_duuid)
            .service(get_new_uuid);
    }
}
...
#[utoipa::path(
    ...
    params(),
    responses(
        (status = 100, description = "Continue", content_type = "application/json", content_type = "application/json", body = Response, content_type = ["application/json", "application/xml"],
            example = json!({"status": StatusCode::CONTINUE.to_string(), "message": ""})),
        (status = 101, description = "Switching Protocols", content_type = "application/json", content_type = "application/json", body = Response, content_type = ["application/json", "application/xml"],
            example = json!({"status": StatusCode::SWITCHING_PROTOCOLS.to_string(), "message": ""})),
        (status = 103, description = "Early Hints", content_type = "application/json", content_type = "application/json", body = Response, content_type = ["application/json", "application/xml"],
...
#[get("/duuid")]
pub async fn get_duuid(
    // _db: Data<DUUIDRepo>,
    _req: HttpRequest,
    // _credentials: BearerAuth,
) -> HttpResponse {
    // let authorization_header = req.headers().get("content-type").unwrap().to_str().unwrap();
    ...
    // // println!("authorization not passed : {}", auth_header.unwrap());
    // match auth_header {
    ...
    // match req.headers().get("Authorization").unwrap_or_default().to_str() {
    //     Ok(uuid) => {
    //         if uuid.eq("") {
    //             return HttpResponse::Unauthorized()
    //             .content_type(contenttype::json())
    ...
    // let uuid = match service::duuid_service::get_duuid() {
    //     Ok(uuid) => uuid,
    //     Err(_e) => {
    ...
    //     Ok(_) => HttpResponse::Ok().json(None),
    //     Err(err) => HttpResponse::InternalServerError().body(err.to_string()),
    // };
    // let db = DUUIDRepo::init();

    // let data = User {
    //     id: None,
    //     name: "new_user.name.to_owned()".to_string(),
    //     location: "new_user.location.to_owned()".to_string(),
    //     title: "new_user.title.to_owned()".to_string(),
    // };
    // db.create_user(data);

    // let resp = model::Response {
    //     message: "".to_string(),
    //     meta: model::Meta { page: 0, limit: 0 },
    //     status: 1,
    // };
    // Ok(web::Json(resp))
}
...
#[get("/uuid")]
pub async fn get_new_uuid(
    db: Data<UUIDRepo>,
    req: HttpRequest,
) -> HttpResponse {
    let accept_header = req.headers().get("accept").unwrap().to_str().unwrap();
    let uuid = match service::uuid_service::get_uuid(db) {
        Ok(uuid) => uuid,
        Err(_e) => {
            return HttpResponse::InternalServerError()
                .content_type(ContentType::json())
