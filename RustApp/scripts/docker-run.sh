...
if [ -n "${IS_START_CONTAINER}" ] && [ "${IS_START_CONTAINER}" == "false" ]; then
    ...
    if [ -n "${LATEST_IMAGE}" ]; then
        echo "docker run -d --platform linux/arm64 --name $IMAGE_NAME --network $NETWORK_NAME --ip $CONTAINER_IP -p $CONTAINER_HTTP_PORT:$HTTP_PORT -p $CONTAINER_TCP_PORT:$TCP_PORT $LATEST_IMAGE";
        # Run your new Docker image
        docker run -d \
            --name $IMAGE_NAME \
            --network $NETWORK_NAME \
