...
if [ -z "${IS_BUILD_MULTIPLATFORM}" ]; then
    echo "The third argument is empty."
    exit 1
fi

if [ -z "${BIN}" ] && [ -n "${IS_BASE_IMAGE}" ] && [ "${IS_BASE_IMAGE}" == "true" ]; then
...
if [ -z "${HOST_PORT}" ] && [ -n "${IS_BASE_IMAGE}" ] && [ "${IS_BASE_IMAGE}" == "true" ]; then
    echo "The eighth argument is empty."
    exit 1
fi

if [ -z "${TCP_PORT}" ] && [ -n "${IS_BASE_IMAGE}" ] && [ "${IS_BASE_IMAGE}" == "true" ]; then
...
if [ -z "${IS_ASSET_IMAGE}" ]; then
    echo "The twelfth argument is empty."
fi

if [ -z "${IS_BASE_IMAGE}" ]; then
...
if [ -n "${IS_ASSET_IMAGE}" ] && [ "${IS_ASSET_IMAGE}" == "true" ]; then
    ...
    # Build the new Docker image
    if [ "${IS_BUILD_MULTIPLATFORM}" == "true" ] && [ "${IS_NO_CACHE}" == "true" ]; then
        docker buildx build \
            --platform ${PLATFORM} \
            ...
            -f build/ci/rust-alpine/Dockerfile \
            -t ${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${IMAGE_NAME}:${next_version}-${TARGET} .
    elif [ "${IS_BUILD_MULTIPLATFORM}" == "true" ]; then
        docker buildx build \
            --platform ${PLATFORM} \
            --build-arg BASE_IMAGE=${rust_alpine_latest_image} \
            --build-arg BIN=${BIN} \
